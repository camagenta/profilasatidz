...
func renderDetailProfile(w http.ResponseWriter, a Asatidz) {
	...
	// Bio section with citation
	bio := a.Bio
	bioFootnote := ""
	if bio == "" {
		bio = "<span class='text-gray-400 text-sm italic'>Belum ada biografi tersedia</span>"
	} else if a.BioSource != "" {
		bioFootnote = fn(a.Sources[0].ID)
		bio = fmt.Sprintf(`<p class="text-sm text-gray-700 leading-relaxed">%s%s</p>`, bio, bioFootnote)
		if a.BioQuote != "" {
			bio += fmt.Sprintf(`<blockquote class="mt-2 pl-3 border-l-2 border-gray-300 text-xs text-gray-500 italic">"%s"</blockquote>`, a.BioQuote)
		}
		bio = `<div>` + bio + `</div>`
	}
	...
	if len(a.Education) > 0 {
		...
		for _, s := range a.Sources {
			if a.EducationSource != "" && s.URL == a.EducationSource {
				srcID = s.ID
				break
			}
		}
		if srcID == "" && len(a.Sources) > 0 {
			srcID = a.Sources[0].ID
		}
		eduFN := ""
		if srcID != "" {
			eduFN = fn(srcID)
		...
		eduSection += `</ul>`
		if a.EducationQuote != "" {
			eduSection += fmt.Sprintf(`<blockquote class="mt-1 pl-3 border-l-2 border-gray-300 text-xs text-gray-500 italic">"%s"</blockquote>`, a.EducationQuote)
		}
		eduSection += `</div>`
	}
	...
	if len(a.Expertise) > 0 {
		srcID := ""
		for _, s := range a.Sources {
			if a.ExpertiseSource != "" && s.URL == a.ExpertiseSource {
				srcID = s.ID
				break
			}
		}
	...
	if len(a.Publications) > 0 {
		srcID := ""
		for _, s := range a.Sources {
			if a.PublicationsSource != "" && s.URL == a.PublicationsSource {
				srcID = s.ID
				break
			}
		}
	...
	// References section (daftar pustaka)
	refSection := ""
	if len(a.Sources) > 0 {
		refSection = `<div class="mt-6 pt-4 border-t border-gray-200"><h3 class="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wider">Referensi (Sanad)</h3><ol class="list-decimal list-inside text-xs text-gray-500 space-y-1">`
		for _, s := range a.Sources {
			title := s.Title
			if title == "" {
				title = s.URL
			}
			refSection += fmt.Sprintf(`<li id="ref-%s"><a href="%s" target="_blank" rel="noopener" class="text-blue-500 hover:underline">%s</a>`, s.ID, s.URL, title)
			if s.Snippet != "" {
				refSection += fmt.Sprintf(` — <span class="text-gray-400 italic">"%s"</span>`, s.Snippet[:min(len(s.Snippet), 120)])
			}
			refSection += `</li>`
		}
		refSection += `</ol></div>`
